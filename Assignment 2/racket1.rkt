...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
    ...
    ((quote-exp? exp) (quote-arg-check exp));added this for excercise 5
    ((if-exp? exp)
     (if (eval-1 (cadr exp))
         (eval-1 (caddr exp))
         (eval-1 (cadddr exp))))
    ((lambda-exp? exp) exp)
    ((map-exp? exp)(map-1 (eval (cadr exp)) (eval (caddr exp))));Added this for exercise 6
    ((and-exp? exp);added this for Excercise 7
     (cond;added this for Excercise 7
       [(= (length exp) 1) '#t];added this for Excercise 7
       [(= (length exp) 2)(eval-1(cadr exp))];added this for Excercise 7
       [(eq? (cadr exp) '#f) (eval-1 (cadr exp))];added this for Excercise 7
       [(eq? (cadr exp) '#t) (eval-1 (caddr exp))];added this for Excercise 7
       [(eq? (eval-1 (cadr exp)) '#t) (eval-1 (caddr exp))];added this for Excercise 7
       [(eq? (eval-1 (cadr exp)) '#f) (display "#f ; ") (cons (caddr exp) '(is not evaluated))];added this for Excercise 7
       [else (error "reached error" exp)]));added this for Excercise 7
    ((pair? exp) (apply-1 (eval-1 (car exp))      ; eval the operator
                  (map eval-1 (cdr exp))))
   
    (else (error "bad expr: " exp))))

(define (map-1 proc arg);added this for Excercise 6
  (map proc arg));added this for Excercise 6
;; Comments on APPLY-1:
...
(define (quote-arg-check arg);added this for Excercise 5
  (if (> (length arg) 2) (error "exp has to be only 1 arguments" arg) (cadr arg) ));added this for Excercise 5
  
(define if-exp? (exp-checker 'if))
(define lambda-exp? (exp-checker 'lambda))
(define and-exp? (exp-checker 'and));added this for Excercise 7
(define map-exp? (exp-checker 'map-1));added this for Excercise 6
